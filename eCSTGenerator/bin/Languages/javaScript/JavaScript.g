...
leftHandSideExpression
	...
	(
		arguments			-> ^( FUNCTION_CALL ^(NAME $leftHandSideExpression) arguments )
...
relationalExpression
	...
	-> shiftExpression ( ^(OPERATOR relationalOperators) shiftExpression )*
	;

relationalExpressionNoIn
	: shiftExpression ( relationalOperatorsNoIn shiftExpression )*
	-> shiftExpression ( ^(OPERATOR relationalOperatorsNoIn) shiftExpression )*
	;
...
equalityExpression
	...
	-> relationalExpression ( ^(OPERATOR equalityOperator) relationalExpression )*
	;
...
logicalANDExpression
	...
	->  bitwiseORExpression ( ^(LOGICAL_OPERATOR LAND) bitwiseORExpression )*
	;

logicalANDExpressionNoIn
	: bitwiseORExpressionNoIn ( LAND bitwiseORExpressionNoIn )*
	-> bitwiseORExpressionNoIn ( ^(LOGICAL_OPERATOR LAND) bitwiseORExpressionNoIn )*
	;
	
logicalORExpression
	: logicalANDExpression ( LOR logicalANDExpression )*
	-> logicalANDExpression ( ^(LOGICAL_OPERATOR LOR) logicalANDExpression )*
	;
	
logicalORExpressionNoIn
	: logicalANDExpressionNoIn ( LOR logicalANDExpressionNoIn )*
	-> logicalANDExpressionNoIn ( ^(LOGICAL_OPERATOR LOR) logicalANDExpressionNoIn )*
	;
...
variableStatement
	...
	-> ^(VAR_DECL_PARENT ^(KEYWORD VAR) variableDeclaration (^(SEPARATOR COMMA) variableDeclaration)*  ^(SEPARATOR semic))
	;

variableDeclaration
	: Identifier ( ASSIGN assignmentExpression )?
	-> ^(VAR_DECL ^(NAME Identifier) ^(TYPE EMPTY)) (^(OPERATOR ASSIGN) assignmentExpression )?
	;
	
variableDeclarationNoIn
	: Identifier ( ASSIGN assignmentExpressionNoIn )?
	-> ^(VAR_DECL ^(NAME Identifier) ^(TYPE EMPTY)) (^(OPERATOR ASSIGN) assignmentExpressionNoIn )?
	;
...
// The predicate is there just to get rid of the warning. ANTLR will handle the dangling else just fine.
	...
	-> ^( BRANCH_STATEMENT ^( BRANCH ^( KEYWORD IF ) ^( CONDITION ^(SEPARATOR LPAREN) expression ^(SEPARATOR RPAREN)  ) statement ) ^( BRANCH ^( KEYWORD ELSE ) statement )? )
	;
...
doStatement
	...
	-> ^( LOOP_STATEMENT ^( KEYWORD DO ) statement ^( CONDITION ^( KEYWORD WHILE ) ^(SEPARATOR LPAREN) expression ^(SEPARATOR RPAREN) ) ^(SEPARATOR semic))
	;
	
whileStatement
	: WHILE LPAREN expression RPAREN statement
	-> ^( LOOP_STATEMENT ^( KEYWORD WHILE ) ^( CONDITION ^(SEPARATOR LPAREN) expression ^(SEPARATOR RPAREN) ) statement )
	;
...
forStatement
	: FOR LPAREN forControl RPAREN statement
	//-> ^(FOR LPAREN forControl RPAREN statement)
	-> ^( LOOP_STATEMENT ^( KEYWORD FOR ) ^( CONDITION ^(SEPARATOR LPAREN) forControl ^(SEPARATOR RPAREN)) statement)
	;
...
forControlVar
	...
	(
		(
			...
			-> ^(KEYWORD VAR) variableDeclarationNoIn ^(KEYWORD IN) expression
		)
		|
		(
			( COMMA variableDeclarationNoIn )* SEMIC ex1=expression? SEMIC ex2=expression?
			//-> variableDeclarationNoIn+ SEMIC $ex1? SEMIC $ex2?
			-> ^(VAR_DECL_PARENT ^(KEYWORD VAR) variableDeclarationNoIn+) ^(SEPARATOR SEMIC) $ex1? ^(SEPARATOR SEMIC) $ex2?
		)
	)
	;
...
formalParameterList
	...
	-> ^(FORMAL_PARAM_LIST ^(SEPARATOR LPAREN) ( ^(PARAMETER_DECL ^(TYPE EMPTY) ^(NAME Identifier)) ( ^(SEPARATOR COMMA) ^(PARAMETER_DECL ^(TYPE EMPTY) ^(NAME Identifier)) )* )? ^(SEPARATOR RPAREN))
	;
